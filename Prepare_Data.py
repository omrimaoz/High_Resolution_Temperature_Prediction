...
def prepare_data(samples, dir=None):
    ...
    for dir in listdir:
        dir_data, station_data, label_data = create_data(dir)

        row_indices = np.random.choice(dir_data['Height'].shape[0], samples)
        col_indices = np.random.choice(dir_data['Height'].shape[0], samples)

        for i, j in zip(row_indices, col_indices):
            data_samples = list()
            for image in dir_data.values():
                data_samples.append(image[i][j])
            for key in STATION_PARAMS_TO_USE:
